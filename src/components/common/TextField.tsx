...
export const TextField = forwardRef<HTMLInputElement, TextFieldProps>(
  (props, ref) => {
    ...
    const base = cn(
      ...
      // error
      hasError ? 'border-red-400 text-red-900 focus:ring-red-500' : '',
      // disabled
    ...
    // size classes
    const sizeClasses =
      resolvedSize === 'md' ? 'px-3 py-2 text-sm' : 'px-2 py-1 text-sm';
      ...
      <div className={cn('flex flex-col', className)}>
        ...
        {hasError && (
          <p
            id={`${inputId}-error`}
            role="alert"
            className="mt-1 text-xs text-red-600  flex items-center gap-2"
          >
